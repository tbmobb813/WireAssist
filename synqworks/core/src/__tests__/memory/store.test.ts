...
describe('MemoryStore.store() — sync path', () => {
  test('stores an entry and returns an id', async () => {
    ...
    const id = store.store({ content: 'hello', agentRole: 'admin', tags: [], createdAt: new Date() });
  ...
  test('entry is retrievable via listRecent', () => {
    const store = freshStore();
    store.store({ content: 'remember this', agentRole: 'admin', tags: ['test'], createdAt: new Date() });
...
describe('MemoryStore.storeAsync()', () => {
  test('stores entry with embedding immediately', async () => {
    const store = freshStore();
    const id = await store.storeAsync({ content: 'async store', agentRole: 'content', tags: [], createdAt: new Date() });
    expect(typeof id).toBe('string');
...
describe('MemoryStore.searchAsync()', () => {
  test('returns results via vector search when embeddings exist', async () => {
    ...
    await store.storeAsync({ content: 'vector search target', agentRole: 'admin', tags: [], createdAt: new Date() });
  ...
  test('falls back to FTS5 when no embeddings exist', async () => {
    ...
    store.store({ content: 'fts fallback test', agentRole: 'admin', tags: [], createdAt: new Date() });
  ...
  test('respects agentRole filter', async () => {
    const store = freshStore();
    await store.storeAsync({ content: 'admin memory', agentRole: 'admin', tags: [], createdAt: new Date() });
    await store.storeAsync({ content: 'content memory', agentRole: 'content', tags: [], createdAt: new Date() });
...
describe('MemoryStore.upgradeEmbeddings()', () => {
  test('backfills entries missing embeddings', async () => {
    ...
    store.store({ content: 'needs embedding', agentRole: 'admin', tags: [], createdAt: new Date() });
  ...
  test('returns { upgraded: 0, total: 0 } when all entries already have embeddings', async () => {
    const store = freshStore();
    await store.storeAsync({ content: 'already embedded', agentRole: 'admin', tags: [], createdAt: new Date() });
    (embed as jest.Mock).mockClear();
    const result = await store.upgradeEmbeddings();
