 ...
 * Uses @synqworks/core's ConversationStore and MessageStore instead of Tauri IPC
 * This provides local-first storage with full integration to @synqworks/core
 */
...
const getDbPath = (): string => {
  ...
  const dbPath = process.env.AIA_DB_PATH ||
 ...
 * Matches the same API as the original database module but uses @synqworks/core for storage
 */
export const coreDatabaseApi = {
  ...
  tags: {
    create: async (data: NewTag): Promise<ApiTag> => {
      throw new Error('Tags not yet implemented in @synqworks/core');
    },
