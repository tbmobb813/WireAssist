...
This guide covers setting up and maintaining APT PPA (Personal Package Archive) and Copr repositories for the AI Assist, enabling automatic package distribution across Ubuntu/Debian and Fedora/RHEL systems respectively.
...
Monitor builds at: `https://launchpad.net/~your_username/+archive/ubuntu/ai-assist`
...
Monitor builds at: `https://copr.fedorainfracloud.org/coprs/your_username/ai-assist/`
...
- UI: https://launchpad.net/~username/+archive/ubuntu/ai-assist
- Logs: Each distro has separate build logs

#### Copr

- UI: https://copr.fedorainfracloud.org/coprs/username/ai-assist/
- API: `copr-cli get-project username/ai-assist`
...
```bash
# Check build log
curl https://launchpad.net/~username/+archive/ubuntu/ai-assist/+buildjobs
...
```bash
# Check build status
copr-cli get-package username/ai-assist ai-assist

# View specific build log
copr-cli status username/ai-assist
...
```bash
copr-cli edit-project username/ai-assist \
...
```bash
# APT PPA
curl -s https://launchpad.net/~username/+archive/ubuntu/ai-assist/+packages | grep -i downloads

# Copr
copr-cli get-project username/ai-assist
```
...
```bash
# Add PPA
sudo add-apt-repository ppa:username/ai-assist

# Update and install
sudo apt update
sudo apt install ai-assist
...
```bash
# Enable Copr
sudo dnf copr enable username/ai-assist

# Install
sudo dnf install ai-assist
...
```bash
flatpak install com.aiassist.app
...
# RPM
rpm --import https://copr.fedorainfracloud.org/coprs/username/ai-assist/repo/fedora-40/pubkey.gpg
```
