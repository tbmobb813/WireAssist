...
npm test 2>&1 | aia analyze "Help fix these failures"

# Get AI help while coding
git diff | aia analyze "Security review"
docker logs app 2>&1 | aia analyze "Why did it crash?"

# Ask questions without leaving terminal
aia chat "How do I optimize this SQL query?"
...
# Install CLI tool
sudo ln -s $(pwd)/cli/target/release/linux-ai-cli /usr/local/bin/aia
```
...
3. **Try the CLI**:
   ```bash
   aia chat "Hello! What can you help me with?"
   ```
...
```bash
# Chat with AI
aia chat "your question"
aia ask "your question"  # 'ask' is alias for 'chat'

# Analyze piped input (🔥 most useful!)
cat file.txt | aia analyze
command 2>&1 | aia analyze "optional context"

# Capture and analyze command output
aia capture "npm test" --ai-analyze
...
```bash
# Debug errors
cargo build 2>&1 | aia analyze "Fix compilation errors"

# Code review
git diff | aia analyze "Review these changes" --gui

# Log analysis
journalctl -n 50 --priority=err | aia analyze

# System diagnostics
docker ps -a | aia analyze "Any container issues?"
```
...
```zsh
cd '/media/nixstation-remote/Dev Storage/Projects/AI Assist - Project/ai-assist'
