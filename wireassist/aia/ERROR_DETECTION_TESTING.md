...
echo "src/App.tsx(42,15): error TS2307: Cannot find module 'missing'." | aia analyze
...
```bash
cargo build 2>&1 | aia analyze "Help me fix these compilation errors"
```

### Example 2: Check TypeScript

```bash
npx tsc --noEmit 2>&1 | aia analyze "Review these TypeScript errors"
```

### Example 3: Python Script Errors

```bash
python script.py 2>&1 | aia analyze "Debug this Python error"
```
...
```bash
# In terminal:
npm run dev 2>&1 | tee /dev/tty | aia analyze --auto
```
...
The `aia analyze` command automatically detects errors in piped content.
