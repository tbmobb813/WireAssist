...
3. Check logs: `~/.config/ai-assist/logs/`
...
free -h              # Memory usage
df -h                # Disk space
top -p $(pgrep -f ai-assist)  # Process resource usage

# Check application logs
tail -f ~/.config/ai-assist/logs/app.log
```
...
4. **Check Database Health**
   ```bash
   # On Linux systems
   sqlite3 ~/.config/ai-assist/conversations.db "PRAGMA integrity_check;"
   ```
...
```bash
# Check database
ls -la ~/.config/ai-assist/

# Check permissions
ls -la ~/.config/ai-assist/conversations.db

# Check disk space
df -h

# Check logs
tail -20 ~/.config/ai-assist/logs/error.log
```
...
2. **Fix Database Permissions**
   ...
   sudo chown $USER:$USER ~/.config/ai-assist/conversations.db
   chmod 600 ~/.config/ai-assist/conversations.db
   ```
...
3. **Repair Database**
   ...
   ```bash
   # Backup first
   cp ~/.config/ai-assist/conversations.db \
      ~/.config/ai-assist/conversations.db.backup
...
4. **Rebuild Database**
   ...
   # Stop application
   # Backup data
   mv ~/.config/ai-assist/conversations.db \
      ~/.config/ai-assist/conversations.db.old
...
5. **Manual Import**
   ...
   ```bash
   # Via CLI
   aia import ~/conversation.json

   # With debugging
   AIA_DEBUG=1 aia import ~/conversation.json
   ```
...
1. **Check Logs**
   ...
   ```bash
   tail -50 ~/.config/ai-assist/logs/app.log
   ```
...
3. **Verify Dependencies**
   ...
   ldd /opt/ai-assist/ai-assist | grep "not found"
   ```
...
2. **Re-index Project**
   ...
   aia index-project /path/to/project
...
1. **Check Profile Database**
   ...
   sqlite3 ~/.config/ai-assist/profiles.db ".tables"

   # List existing profiles
   sqlite3 ~/.config/ai-assist/profiles.db "SELECT * FROM profiles;"
   ```
...
2. **Reset Profile System**
   ...
   ```bash
   # Backup first
   cp ~/.config/ai-assist/profiles.db ~/.config/ai-assist/profiles.db.backup

   # Reset profiles (will recreate default)
   rm ~/.config/ai-assist/profiles.db
   ```
...
1. **Check CLI Tool Installation**
   ...
   export PATH="$PATH:/path/to/ai-assist/cli/target/release"
   ```
...
3. **Permission Issues**
   ...
   env XDG_CURRENT_DESKTOP=GNOME ai-assist
   ```
...
aia health --verbose > debug_report.txt
...
tail -100 ~/.config/ai-assist/logs/*.log >> debug_report.txt
```
