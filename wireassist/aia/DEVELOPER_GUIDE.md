# AI Assist - Developer Guide

Complete API documentation and development guide for the AI Assist.
...
The AI Assist is built using a modern technology stack:
...
export function BranchDialog({ isOpen, onClose, messageId, onCreateBranch }: BranchDialogProps) {
...
export function UsageAnalyticsDashboard({ onClose }: UsageAnalyticsDashboardProps) {
...
```bash
# Basic command
aia <command> [options] [arguments]

# Examples
aia chat
aia "Your question here"
aia export <conversation-id> --format json
aia models list
aia settings get <key>
```
...
- **AppImage**: `bundle/appimage/ai-assist_*.AppImage`
- **DEB**: `bundle/deb/ai-assist_*.deb`
- **RPM**: `bundle/rpm/ai-assist-*.rpm`
...
```bash
# Application logs
~/.config/ai-assist/logs/

# System logs
journalctl -u ai-assist
...
cd Linux-AI-Assistant/ai-assist
