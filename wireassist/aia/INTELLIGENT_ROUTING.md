...
AI Assist features **Intelligent Model Routing** - an automatic system that selects the optimal AI model for each query based on:
...
```typescript
// Classify a query
function classifyQuery(query: string, context: RoutingContext): QueryClassification;

// Select optimal model
function selectOptimalModel(context: RoutingContext, settings: RoutingSettings): RoutingDecision;
...
Intelligent Model Routing is a **killer feature** that sets AI Assist apart from competitors. It provides:
