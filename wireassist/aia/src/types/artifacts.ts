...
export function isPreviewable(language: string, content: string): boolean {
  ...
  if (['react', 'jsx', 'tsx'].includes(lang)) {
    return content.includes('return') || content.includes('function') || content.includes('const');
  }
 ...
 */
export function wrapCodeForPreview(content: string, language: ArtifactLanguage): string {
