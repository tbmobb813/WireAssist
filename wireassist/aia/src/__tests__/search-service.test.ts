...
describe('Search Service', () => {
  ...
  describe('Search Scoring', () => {
    ...
    it('should rank user messages higher than assistant', async () => {
      ...
      const assistantMessages = results.messages.filter((r) => r.item.role === 'assistant');

      if (userMessages.length > 0 && assistantMessages.length > 0) {
        const avgUserScore =
          userMessages.reduce((sum, r) => sum + r.score, 0) / userMessages.length;
        const avgAssistantScore =
          assistantMessages.reduce((sum, r) => sum + r.score, 0) / assistantMessages.length;
  ...
  describe('Search Suggestions', () => {
    ...
    it('should return empty suggestions for no match', async () => {
      const suggestions = await searchService.getSuggestions('nonexistent_xyz_query_abc', 10);
  ...
  describe('Result Format', () => {
    ...
    it('should structure result set properly', async () => {
      ...
      expect(results.total).toBe(results.conversations.length + results.messages.length);
      expect(results.query).toBe('test');
      expect(typeof results.executionTimeMs).toBe('number');
    });
  });
