 ...
 * Verifies that @wireassist/core streaming works with AIA app's provider system
 */
...
describe('Core Streaming Provider', () => {
  ...
  describe('Provider Configuration', () => {
    it('should throw error when OpenAI API key is missing', async () => {
      ...
      const messages: ProviderMessage[] = [{ role: 'user', content: 'Test message' }];

      await expect(provider.generateResponse('conv-1', messages)).rejects.toThrow(
  ...
  describe('Streaming Integration', () => {
    it('should accept onChunk callback', async () => {
      ...
      await expect(provider.generateResponse('conv-1', messages, onChunk)).rejects.toThrow();
    });
  ...
  describe('Conversation Context', () => {
    it('should accept conversation ID', async () => {
      ...
      await expect(provider.generateResponse(conversationId, messages)).rejects.toThrow();
    });
