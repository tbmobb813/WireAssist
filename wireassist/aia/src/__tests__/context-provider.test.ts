...
import type { AIContext } from '@wireassist/core';
...
describe('Context-Aware Provider', () => {
  ...
  describe('Context Injection', () => {
    it('should inject context as system message', async () => {
      const messages: ProviderMessage[] = [{ role: 'user', content: 'What is this project?' }];
  ...
  describe('Message Handling', () => {
    ...
    it('should preserve message content', async () => {
      ...
      expect(mockProvider.lastMessages.some((m) => m.content.includes('Test message'))).toBe(true);
    });
    ...
    it('should handle very long messages', async () => {
      ...
      const messages: ProviderMessage[] = [{ role: 'user', content: longContent }];
  ...
  describe('Error Handling', () => {
    it('should handle wrapped provider errors', async () => {
      ...
      await expect(contextAware.generateResponse('conv-123', messages)).rejects.toThrow(
        'Provider error'
      );
    });
  ...
  describe('Context Message Formatting', () => {
    it('should format file context properly', async () => {
      const messages: ProviderMessage[] = [{ role: 'user', content: 'Explain this code' }];
