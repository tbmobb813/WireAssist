...
describe('settingsStore', () => {
  ...
  afterEach(() => {
    ...
    try {
      settingsModule.useSettingsStore.getState().setProjectRoot = async () => {};
    } catch {}
  });
  ...
  it('registerGlobalShortcut shows success toast when registration succeeds', async () => {
    ...
    settingsModule = await import('../lib/stores/settingsStore');
    // call the action
    await settingsModule.useSettingsStore.getState().registerGlobalShortcut('Ctrl+K');
  ...
  it('registerGlobalShortcut adds error toast when registration throws', async () => {
    const tauri = await import('../lib/utils/tauri');
    (tauri.unregisterAllShortcutsSafe as any).mockResolvedValue(undefined);
    (tauri.registerGlobalShortcutSafe as any).mockRejectedValue(new Error('boom'));

    settingsModule = await import('../lib/stores/settingsStore');
    await settingsModule.useSettingsStore.getState().registerGlobalShortcut('Ctrl+Z');
