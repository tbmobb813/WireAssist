...
describe('provider streaming via listen', () => {
  it('openai streaming path collects chunks and returns buffer', async () => {
    ...
    vi.doMock('../lib/tauri-shim', () => ({
      ...
      getListen: async () => async (event: string, cb: Function) => {
        ...
        if (event === 'provider-stream-chunk') {
          ...
          setTimeout(() => cb({ payload: { session_id: 'sess-123', chunk: 'hello' } }), 0);
          setTimeout(() => cb({ payload: { session_id: 'sess-123', chunk: ' world' } }), 0);
        }
    ...
    const chunks: string[] = [];
    const p = prov.generateResponse('c-stream', [{ role: 'user', content: 'x' }], (c) =>
      chunks.push(c)
    );
  ...
  it('ollama streaming path collects chunks and returns buffer', async () => {
    vi.doMock('../lib/tauri-shim', () => ({
      ...
      getListen: async () => async (event: string, cb: Function) => {
        ...
        if (event === 'provider-stream-chunk') {
          setTimeout(() => cb({ payload: { session_id: 'sess-ollama', chunk: 'A' } }), 0);
          setTimeout(() => cb({ payload: { session_id: 'sess-ollama', chunk: 'B' } }), 0);
        }
        if (event === 'provider-stream-end') {
    ...
    const chunks: string[] = [];
    const p = prov.generateResponse('c-olla', [{ role: 'user', content: 'x' }], (c) =>
      chunks.push(c)
    );
