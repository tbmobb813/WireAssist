...
describe('Settings (global shortcut)', () => {
  ...
  test('shows validation error for invalid value', async () => {
    ...
    expect(await screen.findByText(/Use format like CommandOrControl\+Space/i)).toBeTruthy();
  });
  ...
  test('clears validation message when corrected', async () => {
    ...
    // Error appears
    const errorEl = await screen.findByText(/Use format like CommandOrControl\+Space/i);
    ...
    await waitFor(() => {
      const maybeError = screen.queryByText(/Use format like CommandOrControl\+Space/i);
