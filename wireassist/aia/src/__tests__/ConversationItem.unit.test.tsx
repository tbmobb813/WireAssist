...
describe('ConversationItem (unit)', () => {
  ...
  it('calls onSelect when clicked', () => {
    ...
    const { getByText } = render(
      <ConversationItem conversation={conv} selected={false} onSelect={onSelect} />
    );
  ...
  it('rename success shows toast', async () => {
    ...
    const { getByText, getByDisplayValue } = render(
      <ConversationItem conversation={conv} selected={false} onSelect={vi.fn()} />
    );
    ...
    await waitFor(() => expect(mockUpdate).toHaveBeenCalledWith('c1', 'New Title'));
