 ...
 * Verifies that @wireassist/core adapters work correctly with the AIA app
 */
...
describe.sequential('Core Database Adapters', () => {
  ...
  describe('Adapter Integration', () => {
    ...
    it('should handle multiple conversations and messages', async () => {
      ...
      expect(messages1.every((m) => m.conversation_id === conv1.id)).toBe(true);
      expect(messages2.every((m) => m.conversation_id === conv2.id)).toBe(true);
    });
  });
});
