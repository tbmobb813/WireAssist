import { lazy, useEffect, useState, Suspense, startTransition, useCallback } from 'react';
...
const ExecutionAuditModal = lazy(() => import('./components/ExecutionAuditModal'));
const CommandSuggestionsModal = lazy(() => import('./components/CommandSuggestionsModal'));
const Settings = lazy(() => import('./components/SettingsTabs'));
const UpdateManager = lazy(() => import('./components/UpdateManager'));
const ProjectContextPanel = lazy(() => import('./components/ProjectContextPanel'));
...
export default function App(): JSX.Element {
  const { loadSettings, registerGlobalShortcut, globalShortcut, theme } = useSettingsStore();
      ...
      try {
        ...
        unlistenCliAsk = await mod.listen<any>('cli://ask', async (e) => {
          try {
            ...
            const provider = targetProvider || settings.defaultProvider || 'local';
            if (!convo || forceNew) {
        ...
        // project file events
        unlistenProject = await mod.listen<string[]>('project://file-event', (e) => {
  ...
  useEffect(() => {
    ...
    return () => document.removeEventListener('open-settings', handler as EventListener);
  }, []);
            ...
            >
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400 hidden md:block">
                  {useChatStore((state) => state.currentConversation?.title) || 'No conversation'}
                </div>
                ...
                >
                  ...
                  />
                  <span className="tracking-wide">CMD+K: {isOpen ? 'OPEN' : 'CLOSED'}</span>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <AnimatedButton
                  ...
                  variant={useReviewStore((s) => s.isReviewMode) ? 'primary' : 'secondary'}
                  size="sm"
                >
                ...
                <AnimatedButton
                  ...
                  variant={useLogStore((s) => s.isLogViewerOpen) ? 'primary' : 'secondary'}
                  size="sm"
                >
                  <span className="text-base">📋</span>
                ...
                <AnimatedButton
                  ...
                  variant={useMemoryStore((s) => s.isMemoryViewerOpen) ? 'primary' : 'secondary'}
                  size="sm"
                >
                  <span className="text-base">🧠</span>
                ...
                >
                  ...
                  {events.filter((event) => Date.now() - event.ts < 5 * 60 * 1000).length > 0 && (
                ...
                <div title="Toggle Focus Mode (F11)">
                  <AnimatedButton
                    ...
                    variant={useUiStore((s) => s.focusMode) ? 'primary' : 'secondary'}
                    size="sm"
                  >
          ...
          {showProjectContext && (
            <Suspense fallback={null}>
              <div className="absolute right-6 top-20 z-50">
                <ProjectContextPanel onClose={() => setShowProjectContext(false)} />
              </div>
            </Suspense>
          )}
