 ...
 */
export function classifyQuery(query: string, context: RoutingContext): QueryClassification {
  ...
  for (const [type, patterns] of Object.entries(QUERY_TYPE_PATTERNS)) {
    const score = patterns.filter((pattern) => lowerQuery.includes(pattern)).length;
    if (score > maxScore) {
      maxScore = score;
  ...
  if (COMPLEXITY_INDICATORS.complex.some((indicator) => lowerQuery.includes(indicator))) {
    complexity = 'complex';
  } else if (COMPLEXITY_INDICATORS.simple.some((indicator) => lowerQuery.includes(indicator))) {
    complexity = 'simple';
  }
  ...
  const tokensEstimate = Math.ceil((query.length + (requiresContext ? 2000 : 0)) / 4);
...
function scoreModel(
  ...
  if (model.maxComplexity === classification.complexity) {
    score += 25;
  } else if (model.maxComplexity === 'complex' && classification.complexity === 'medium') {
    score += 20; // Can handle it, but might be overkill
  } else if (model.maxComplexity === 'simple' && classification.complexity !== 'simple') {
...
export function selectOptimalModel(
  ...
  if (!settings.enabled || context.userPreference) {
    ...
    return {
      modelId,
      reasoning: context.userPreference ? 'User preference' : 'Routing disabled',
      confidence: 1.0,
      alternatives: [],
    };
  }
  ...
  if (context.projectType && settings.alwaysUseForLanguages?.[context.projectType]) {
  ...
  const reasoning =
    reasons.length > 0 ? `Best for: ${reasons.join(', ')}` : 'General purpose model';
