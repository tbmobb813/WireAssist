...
export class HybridRoutingProvider implements HybridProvider {
  private cachedOllamaCheck: { timestamp: number; available: boolean } | null = null;
  ...
  async getAvailableProviders() {
    ...
    if (this.cachedOllamaCheck && now - this.cachedOllamaCheck.timestamp < this.CACHE_DURATION) {
      ollamaAvailable = this.cachedOllamaCheck.available;
    } else {
      try {
        ollamaAvailable = (await invokeSafe<boolean>('ollama_check_connection')) || false;
  ...
  async chooseProvider(requestedModel?: string): Promise<{
    ...
    if (availability.cloud.available) {
      ...
      const model = requestedModel || this.getDefaultModelForProvider(cloudProvider);
  ...
  async generateResponse(
    ...
    try {
      ...
      const result = await regularProvider.generateResponse(conversationId, messages, onChunk);
