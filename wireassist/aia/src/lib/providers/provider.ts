...
import { getInvoke as getTauriInvoke, getListen as getTauriListen, hasTauri } from '../tauri-shim';
    ...
    async generateResponse(conversationId, messages, onChunk?) {
      try {
        ...
        if (!isRuntimeTauri || !invokeFn) {
          ...
          return mockProvider.generateResponse(conversationId, messages, onChunk);
        }

        const listenAvailable =
          isRuntimeTauri && listenFn !== undefined && typeof listenFn === 'function';
        ...
        if (provider === 'openai') {
          if (onChunk && listenAvailable) {
            ...
            let buffer = '';
            const unlistenChunkP = listenFn('provider-stream-chunk', (e: any) => {
