...
import { getAllProfiles, getActiveProfile, setActiveProfile } from './api/profiles';
...
export const slashCommands: SlashCommand[] = [
    ...
    handler: async (args, context) => {
      ...
      try {
        ...
        if (isTauriEnvironment()) {
          ...
          const conversation = await database.conversations.get(context.conversationId);
  ...
  },
  {
    ...
    handler: async (_args, context) => {
      ...
      const conversationId = context.conversationId || (await createNewConversation());
    ...
    handler: async (args, context) => {
      ...
      try {
        if (isTauriEnvironment()) {
          ...
          const searchResult = await invokeSafe<SearchResult>('search_project_files', {
            query,
          ...
          if (searchResult && searchResult.matches && searchResult.matches.length > 0) {
            const { matches, total_files_searched, search_time_ms } = searchResult;
...
To execute the command \`${command}\`, use the AI Assist CLI:
...
# With error analysis
aia capture "${command}" --analyze

# With AI-powered analysis
aia capture "${command}" --ai-analyze

# With custom timeout (default: 30s)
aia capture "${command}" --timeout 60
\`\`\`
...
The CLI \`capture\` command provides safe execution with comprehensive output analysis, perfect for debugging build scripts, testing commands, and analyzing errors.`;
  ...
  {
    command: 'profile',
    description: 'Switch between conversation profiles or list available profiles',
    ...
    handler: async (args, context) => {
      try {
        if (args.length === 0) {
          ...
          const targetProfile = profiles.find((p) => p.name.toLowerCase() === profileName);
