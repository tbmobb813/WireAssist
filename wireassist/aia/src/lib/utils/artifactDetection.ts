 ...
 */
export function detectArtifacts(messageId: string, content: string): Artifact[] {
  const codeBlocks = extractCodeBlocks(content);
  const artifacts: Artifact[] = [];

  codeBlocks.forEach((block: { language: string; content: string }, index: number) => {
    ...
    if (previewable || block.content.split('\n').length > 5) {
      ...
      const titleMatch = block.content.match(/^(?:\/\/|#|\/\*|\<\!--)\s*(.+?)(?:\*\/|-->)?$/ms);
      if (titleMatch) {
