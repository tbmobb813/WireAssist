...
const ERROR_MESSAGES: Record<string, string> = {
  // Network errors
  fetch_failed: 'Unable to connect to the service. Please check your internet connection.',
  timeout: 'The request timed out. Please try again.',
  connection_refused: 'Cannot connect to the service. It may be temporarily unavailable.',

  // Database errors
  db_connection_failed: 'Database connection failed. Your data may not be saved.',
  db_query_failed: 'Failed to retrieve data. Please try refreshing the application.',
  ...
  disk_full: 'Insufficient disk space. Please free up some space and try again.',
...
export function createAppError(error: any, context?: string, customMessage?: string): AppError {
  ...
  return {
    ...
    userMessage: customMessage || ERROR_MESSAGES[errorKey] || ERROR_MESSAGES.unknown_error,
    details: error,
    timestamp: Date.now(),
    context,
...
function extractErrorKey(error: any): string {
  ...
  if (errorString.includes('fetch') && errorString.includes('failed')) return 'fetch_failed';
  ...
  if (errorString.includes('disk full') || errorString.includes('no space')) return 'disk_full';
...
export class ErrorHandler {
  ...
  private static showErrorToast(error: AppError, recoveryActions?: ErrorRecovery[]): void {
