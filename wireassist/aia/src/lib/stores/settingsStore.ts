...
import { registerGlobalShortcutSafe, unregisterAllShortcutsSafe } from '../utils/tauri';
...
export const useSettingsStore = create<SettingsState>((set) => ({
  ...
  loadSettings: async () => {
    try {
      ...
      const globalShortcut = (await db.settings.get('globalShortcut')) || 'CommandOrControl+Space';
      ...
      const ignorePatterns = await db.settings.getJSON<string[]>('fileWatcherIgnorePatterns');
  ...
  registerGlobalShortcut: async (shortcutOptional) => {
    const shortcut = shortcutOptional || useSettingsStore.getState().globalShortcut;
