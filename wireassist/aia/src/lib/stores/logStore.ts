 ...
 */
export type LogFormat = 'syslog' | 'docker' | 'nginx' | 'apache' | 'json' | 'generic';
...
function detectLogFormat(rawLogs: string): LogFormat {
  ...
  if (/^[A-Z][a-z]{2}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}\s+\S+\s+\S+/.test(firstLines)) {
    return 'syslog';
  }
  ...
  return lines
    .map((line, idx) => {
      ...
      switch (detectedFormat) {
        case 'json':
          return parseJsonLog(line, lineNumber) || parseGenericLog(line, lineNumber);
        case 'docker':
    ...
    (set, get) => ({
      ...
      createSession: (title, rawLogs, format) => {
        ...
        const session: LogSession = {
          ...
          filters: {
            levels: ['trace', 'debug', 'info', 'warn', 'error', 'fatal', 'unknown'],
            search: '',
          },
        };
      ...
      deleteSession: (sessionId) => {
        set((state) => ({
          ...
          isLogViewerOpen: state.currentSession?.id === sessionId ? false : state.isLogViewerOpen,
        }));
      },
          ...
          currentSession:
            state.currentSession?.id === sessionId
              ? {
                  ...
                  collapsedLevels: (() => {
                    const collapsed = new Set(state.currentSession.collapsedLevels);
                    if (collapsed.has(level)) {
                      collapsed.delete(level);
                    } else {
                      collapsed.add(level);
