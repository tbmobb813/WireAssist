...
interface BranchStore {
  ...
  mergeBranch: (conversationId: string, sourceBranchId: string, targetBranchId: string) => void;
  ...
  getBranchPath: (conversationId: string, branchId: string) => ConversationBranch[];
}
    ...
    (set, get) => ({
      ...
      renameBranch: (branchId, newName) => {
        set((state) => {
          ...
          for (const [conversationId, conversationMeta] of metadata.entries()) {
            const branchIndex = conversationMeta.branches.findIndex((b) => b.id === branchId);
      ...
      mergeBranch: (conversationId, sourceBranchId, targetBranchId) => {
        set((state) => {
          ...
          const sourceBranch = conversationMeta.branches.find((b) => b.id === sourceBranchId);
          const targetBranch = conversationMeta.branches.find((b) => b.id === targetBranchId);

          if (!sourceBranch || !targetBranch) return state;

          // Merge message IDs from source to target
          targetBranch.messageIds = [...targetBranch.messageIds, ...sourceBranch.messageIds];
      ...
      getActiveBranch: (conversationId) => {
        ...
        return metadata.branches.find((b) => b.id === metadata.activeBranchId) || null;
      },
      ...
      getBranchPath: (conversationId, branchId) => {
        ...
        while (currentBranchId) {
          const branch = metadata.branches.find((b) => b.id === currentBranchId);
          if (!branch) break;
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state && Array.isArray(state.branchMetadata)) {
          state.branchMetadata = new Map(state.branchMetadata as [string, BranchMetadata][]);
        }
      },
    }
  )
