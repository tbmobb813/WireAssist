...
export const useUiStore = create<UiState>((set) => ({
  ...
  addToast: (t) => {
    ...
    if (toast.ttl && toast.ttl > 0) {
      setTimeout(() => set((s) => ({ toasts: s.toasts.filter((x) => x.id !== id) })), toast.ttl);
    }
    return id;
  },
  ...
  closeAudit: () => set(() => ({ auditModal: { open: false, content: '', loading: false } })),
  suggestionsModal: {
    open: false,
    items: [],
  },
  showSuggestions: (items: string[]) => set(() => ({ suggestionsModal: { open: true, items } })),
  closeSuggestions: () => set(() => ({ suggestionsModal: { open: false, items: [] } })),
  apiKeyModal: {
    open: false,
    title: '',
    onSubmit: undefined,
