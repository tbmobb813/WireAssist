...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  sendMessage: async (content) => {
    ...
    try {
      ...
      if (routingStore.settings.enabled || routingStore.manualOverride) {
        try {
          ...
          const routingContext = {
            ...
            hasErrorContext:
              content.toLowerCase().includes('error') || content.toLowerCase().includes('fix'),
            previousModel: selectedModelId || undefined,
            userPreference: routingStore.manualOverride || undefined,
          };

          // Get routing decision
          const decision = selectOptimalModel(routingContext, routingStore.settings);
          ...
          routingStore.addRoutingDecision(decision, classification.type, projectType);
      ...
      try {
        const summary = useProjectStore.getState().getRecentSummary(8, 2 * 60 * 1000);
        if (summary) {
      ...
      try {
        ...
        set((state) => ({
          messages: state.messages.map((m) =>
            m.id === optimisticAssistantId ? { ...assistantMessage, status: 'sent' } : m
          ),
          isLoading: false,
        }));
