...
function exportReviewAsMarkdown(session: ReviewSession): string {
  ...
  for (const annotation of session.annotations) {
    ...
    for (const file of session.files) {
      for (const hunk of file.hunks) {
        const lineExists = hunk.lines.some((line) => line.annotation?.id === annotation.id);
        if (lineExists) {
  ...
  markdown += '---\n\n';
  markdown += `*Generated by AI Assist - Code Review Mode*\n`;
  ...
  persist(
    (set, get) => ({
              ...
              hunks: file.hunks.map((hunk) => {
                ...
                return {
                  ...
                  lines: hunk.lines.map((line, idx) =>
                    idx === annotation.lineIndex ? { ...line, annotation: newAnnotation } : line
                  ),
                };
              }),
            }));
      ...
      deleteAnnotation: (sessionId, annotationId) => {
        set((state) => ({
          sessions: state.sessions.map((session) => {
            ...
            return {
              ...session,
              annotations: session.annotations.filter((ann) => ann.id !== annotationId),
              updatedAt: Date.now(),
            };
          }),
        }));
      ...
      completeSession: (sessionId) => {
        set((state) => ({
          sessions: state.sessions.map((s) =>
            s.id === sessionId ? { ...s, status: 'complete' as const, updatedAt: Date.now() } : s
          ),
        }));
      },
      ...
      deleteSession: (sessionId) => {
        set((state) => ({
          ...
          isReviewMode: state.currentSession?.id === sessionId ? false : state.isReviewMode,
        }));
      },
    }),
    {
      name: 'ai-assist-review',
      partialize: (state) => ({
