...
const calculateSimilarity = (
  ...
  const normalizedTfidf = maxPossibleScore > 0 ? tfidfScore / maxPossibleScore : 0;

  // 2. Context matching bonus
  let contextScore = 0;
  if (contextWords.length > 0) {
    const contextMatches = queryWords.filter((qWord) =>
      contextWords.some((cWord) => cWord.includes(qWord) || qWord.includes(cWord))
    );
  ...
  persist(
    (set, get) => ({
      ...
      addMemory: (content, options = {}) => {
        ...
        if (projectMemory && projectMemory.totalTokens > projectMemory.maxTokens) {
      ...
      removeMemory: (memoryId) => {
        set((state) => {
          ...
          for (const [projectPath, projectMemory] of memories.entries()) {
            const entryIndex = projectMemory.entries.findIndex((e) => e.id === memoryId);
        ...
        set((state) => {
          ...
          for (const [projectPath, projectMemory] of memories.entries()) {
            ...
            if (entry) {
              ...
              projectMemory.totalTokens = projectMemory.totalTokens - oldTokens + newTokens;
              memories.set(projectPath, projectMemory);
              break;
            }
          }
