 ...
 * Uses @wireassist/core's ConversationStore and MessageStore instead of Tauri IPC
 * This provides local-first storage with full integration to @wireassist/core
 */

import {
  initializeCoreAdapters,
...
const getDbPath = (): string => {
  ...
  const dbPath =
    process.env.AIA_DB_PATH ||
    `${process.env.HOME || process.env.USERPROFILE || '/tmp'}/.aia/conversations.db`;
  return dbPath;
};
 ...
 * Matches the same API as the original database module but uses @wireassist/core for storage
 */
export const coreDatabaseApi = {
  ...
  tags: {
    create: async (data: NewTag): Promise<ApiTag> => {
      throw new Error('Tags not yet implemented in @wireassist/core');
    },
