  ...
  if (!isTauriEnvironment()) {
    switch (cmd) {
      ...
      case 'get_all_settings': {
        ...
        wps.map.forEach((v: { value: string; updated_at: number }, key: string) => {
      ...
      case 'create_workspace_template':
        return {
          ...
          default_provider: (args as any)?.template?.default_provider ?? 'local',
...
export const database = {
  ...
  tags: {
    ...
    addBulkToConversation: async (conversationId: string, tagNames: string[]): Promise<Tag[]> => {
  ...
  workspaceTemplates: {
    ...
    getByCategory: async (category: string): Promise<WorkspaceTemplate[]> => {
      return callInvoke<WorkspaceTemplate[]>('get_workspace_templates_by_category', { category });
    },
...
export async function enableCoreDatabase() {
  try {
    ...
    console.log('[Database] Switched to @wireassist/core-backed database');
    return true;
  } catch (error) {
