...
export default function ProfileSettings({ onClose }: ProfileSettingsProps) {
  ...
  const [activeProfile, setActiveProfileState] = useState<ApiProfile | null>(null);
    ...
    <div className="w-[480px] max-h-[600px] bg-[#1a1b26] border border-[#414868] rounded-lg shadow-2xl p-6 overflow-y-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-[#c0caf5]">Profile Settings</h2>
        </div>
        <button
          onClick={onClose}
      ...
      {activeProfile && (
        <div className="mb-6 p-4 bg-[#7aa2f7]/10 border border-[#7aa2f7]/30 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-[#c0caf5]">Active Profile</span>
          </div>
          <div className="text-sm text-[#c0caf5]">
            <div className="font-medium">{activeProfile.name}</div>
            {activeProfile.description && (
              <div className="text-xs text-[#9aa5ce] mt-1">{activeProfile.description}</div>
            )}
      ...
      <div className="mb-6 p-4 bg-[#24283b]/50 border border-[#414868]/50 rounded-lg">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-medium text-[#c0caf5]">Create New Profile</span>
        </div>
              ...
              <div>
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-[#c0caf5]">{profile.name}</span>
                      {profile.is_active && <Check className="w-4 h-4 text-[#9ece6a]" />}
                    </div>
                    {profile.description && (
                      <div className="text-xs text-[#9aa5ce] mt-1">{profile.description}</div>
                    )}
                    <div className="text-xs text-[#565f89] mt-1">
