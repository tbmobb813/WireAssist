...
export const CommandPalette: React.FC<CommandPaletteProps> = ({ isOpen, onClose }) => {
  ...
  const commands = useMemo((): Command[] => {
    ...
    const actionCommands: Command[] = [
      ...
      },
      {
        ...
        subtitle: 'View and manage conversation branches — Shortcut: Ctrl+Shift+B',
        category: 'actions',
        icon: '🌲',
        action: async () => {
        ...
        keywords: ['branch', 'fork', 'tree', 'conversation', 'alternative', 'parallel'],
      },
    ];
  ...
  }, [conversations, selectConversation, createConversation, loadConversations, onClose]);
  ...
  const filteredCommands = useMemo(() => {
    ...
    return commands
      .filter((command) => {
        const searchText = [command.title, command.subtitle, ...(command.keywords || [])]
          .join(' ')
          .toLowerCase();
        ...
        <div className="max-h-96 overflow-y-auto bg-white dark:bg-gray-900">
          {filteredCommands.length === 0 ? (
            <div className="p-8 text-center text-gray-500 dark:text-gray-400">
              ...
              <div className="text-sm">Try searching for conversations, actions, or commands</div>
            </div>
          ) : (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        {command.category === 'conversations' &&
                          currentConversation?.id === command.id.replace('conv-', '') && (
