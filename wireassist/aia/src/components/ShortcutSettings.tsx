...
import { Keyboard, X, Check, AlertCircle, Power, PowerOff, Info } from 'lucide-react';
...
export default function ShortcutSettings({ onClose }: ShortcutSettingsProps) {
  ...
  const updateShortcut = async (action: keyof ShortcutAction, newShortcut: string) => {
    if (!config) return;
    ...
    try {
      ...
      const newConfig = {
        ...config,
        shortcuts: config.shortcuts.map((shortcut) =>
          shortcut.action === action ? { ...shortcut, shortcut: newShortcut } : shortcut
        ),
      };
  ...
  const startEditing = (action: keyof ShortcutAction, currentShortcut: string) => {
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-[#414868]">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-[#c0caf5]">Global Shortcuts</h2>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      <div className="p-4">
        ...
        <div className="mb-6 p-3 bg-[#7aa2f7]/10 border border-[#7aa2f7]/30 rounded-lg">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-[#c0caf5]">
              <strong>Tip:</strong> Click on a shortcut to edit it. Use standard keyboard notation
              like "CommandOrControl+Space" or "Alt+Shift+N".
            </div>
          </div>
        </div>
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          className={`p-1 rounded transition-colors ${
                            shortcut.enabled ? 'text-[#9ece6a]' : 'text-[#565f89]'
                          }`}
                    ...
                    <div className="flex items-center gap-2">
                      {editingShortcut === shortcut.action ? (
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <button
                            onClick={() => updateShortcut(shortcut.action, tempShortcut)}
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => startEditing(shortcut.action, shortcut.shortcut)}
        ...
        {editingShortcut && (
          <div className="mt-4 p-3 bg-[#e0af68]/10 border border-[#e0af68]/30 rounded-lg">
            <div className="text-sm text-[#c0caf5]">
              <strong>Editing mode:</strong> Press the key combination you want to use, or type it
              manually (e.g., "CommandOrControl+Shift+Space").
            </div>
          </div>
        )}
      </div>
