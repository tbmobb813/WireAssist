...
import { getModelIcon, getModelDisplayName } from '../lib/services/routingService';

export default function RoutingIndicator() {
  const { currentDecision, manualOverride, setManualOverride, settings } = useRoutingStore();
  ...
  const activeModelId = manualOverride || currentDecision?.modelId || settings.fallbackModel;
          ...
          <div className="absolute bottom-full mb-2 left-0 min-w-[280px] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-20 py-1">
            ...
            {MODEL_CAPABILITIES.map((model) => {
              ...
              return (
                ...
                >
                  <span className="text-lg leading-none mt-0.5">{getModelIcon(model.modelId)}</span>
