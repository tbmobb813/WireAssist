...
export default function SettingsTabs({ onClose }: Props): JSX.Element {
  ...
  const [showWindowPositionSettings, setShowWindowPositionSettings] = useState(false);
  ...
  const filteredContent = useMemo(() => {
    ...
    const searchMap: Record<TabId, string[]> = {
      general: ['shortcut', 'global', 'keyboard', 'hotkey', 'execution', 'code'],
      models: ['provider', 'api', 'key', 'openai', 'anthropic', 'model', 'gpt', 'claude'],
    ...
    <div className="w-[800px] h-[600px] bg-[#1a1b26] border border-[#414868] rounded-xl shadow-2xl shadow-black/40 text-[#c0caf5] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#9aa5ce]">Configure your preferences</p>
            </div>
          </div>
          {onClose && (
            <button
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto smooth-scroll">
          {searchQuery && filteredContent ? (
            <SearchResults results={filteredContent} onTabSelect={setActiveTab} />
          ) : (
            <div className="p-6">
              ...
              {activeTab === 'advanced' && (
                <AdvancedTab
                  ...
                  onShowWindowSettings={() => setShowWindowPositionSettings(true)}
      ...
      {showWindowPositionSettings && (
        <ModalWrapper onClose={() => setShowWindowPositionSettings(false)}>
          <WindowPositionSettings onClose={() => setShowWindowPositionSettings(false)} />
        </ModalWrapper>
      )}
      {showFileWatcherSettings && (
        <ModalWrapper onClose={() => setShowFileWatcherSettings(false)}>
          <FileWatcherSettings onClose={() => setShowFileWatcherSettings(false)} />
        </ModalWrapper>
      )}
      {showDocumentSearch && (
      ...
      {showUsageAnalytics && (
        <ModalWrapper onClose={() => setShowUsageAnalytics(false)}>
          <UsageAnalyticsDashboard onClose={() => setShowUsageAnalytics(false)} />
        </ModalWrapper>
      )}
    </div>
  );
...
function ModalWrapper({ children }: { children: React.ReactNode; onClose: () => void }) {
  return (
...
function SearchResults({
  ...
  return (
    <div className="p-6 space-y-4">
      <h3 className="text-sm font-semibold text-[#c0caf5] mb-4">Search Results</h3>
      {results.map((result, idx) => (
        <button
          ...
          "
        >
          <div className="font-medium text-[#c0caf5] capitalize">{result.setting}</div>
...
function GeneralTab() {
  ...
  const validate = (s: string): string | null => {
    ...
    const hasModifier = /(Command|Control|Ctrl|Cmd|Alt|Option|Shift|Super|Meta)/i.test(s);
    const hasKey = /\+\s*[^+\s]+$/i.test(s);
    if (!hasModifier || !hasKey) return 'Use format like CommandOrControl+Space or Ctrl+Shift+K';
    return null;
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-sm font-semibold text-[#c0caf5] mb-4">General Settings</h3>

        {/* Global Shortcut */}
        <div className="bg-[#24283b] border border-[#414868] rounded-lg p-4 space-y-3">
          <label className="block text-sm font-medium text-[#c0caf5]">Global Shortcut</label>
          ...
          <p className="text-xs text-[#9aa5ce]">Examples: CommandOrControl+Space, Ctrl+Shift+K</p>
          {error && (
            <p className="text-xs text-[#f7768e] bg-[#f7768e]/10 px-2 py-1 rounded">{error}</p>
          )}
        </div>
        ...
        <div className="mt-4 bg-[#24283b] border border-[#414868] rounded-lg p-4 space-y-3">
          <label className="block text-sm font-medium text-[#c0caf5]">Monthly Budget (USD)</label>
          <p className="text-xs text-[#9aa5ce]">
            Set a monthly spending limit for AI usage. You'll get alerts when approaching or
...
// Models Tab
function ModelsTab() {
  const { defaultProvider, setDefaultProvider, defaultModel, setDefaultModel, apiKeys, setApiKey } =
    useSettingsStore();
  const addToast = useUiStore((s) => s.addToast);
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-sm font-semibold text-[#c0caf5]">Model Configuration</h3>

      {/* Provider Selection */}
      <div className="bg-[#24283b] border border-[#414868] rounded-lg p-4 space-y-3">
        <label className="block text-sm font-medium text-[#c0caf5]">Default Provider</label>
        <select
      ...
      {/* Model Selection */}
      <div className="bg-[#24283b] border border-[#414868] rounded-lg p-4 space-y-3">
        <label className="block text-sm font-medium text-[#c0caf5]">Default Model</label>
        <select
          value={selectedModel}
...
function AppearanceTab() {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-sm font-semibold text-[#c0caf5]">Appearance Settings</h3>

      {/* Theme Selection */}
      <div className="bg-[#24283b] border border-[#414868] rounded-lg p-4 space-y-3">
        <label className="block text-sm font-medium text-[#c0caf5]">Theme</label>
        ...
        <p className="text-xs text-[#9aa5ce]">System mode follows your OS dark mode preference</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        {advancedOptions.map((option, idx) => (
          ...
          >
            ...
            <span className="text-sm font-medium text-[#c0caf5]">{option.label}</span>
          </button>
        ))}
      </div>
      ...
      >
        ...
        <span className="text-sm font-medium text-[#c0caf5]">Usage Analytics Dashboard</span>
      </button>
      ...
      >
        ...
        <span className="text-sm font-medium text-[#c0caf5]">Replay Onboarding Tour</span>
      </button>
    </div>
  );
}
