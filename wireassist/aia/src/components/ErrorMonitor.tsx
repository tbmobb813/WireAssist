...
export default function ErrorMonitor() {
  ...
  useEffect(() => {
    ...
    // Show new errors
    const newErrors = errors.filter((e) => !visibleErrors.includes(e.id)).slice(0, 3); // Max 3 notifications at once

    if (newErrors.length > 0) {
      setVisibleErrors((prev) => [...newErrors.map((e) => e.id), ...prev].slice(0, 3));
    }
  }, [errors, isEnabled]);
      ...
      {visibleErrors.map((errorId) => {
        ...
        return (
          <FadeIn key={error.id} className="pointer-events-auto">
            <ErrorNotification error={error} onDismiss={() => handleDismiss(error.id)} />
          </FadeIn>
        );
      })}
    </div>
