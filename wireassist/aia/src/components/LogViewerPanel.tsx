...
import { useLogStore, LogEntry, LogLevel, LogSession } from '../lib/stores/logStore';
...
const exportAsJSON = (entries: LogEntry[], title: string) => {
  const data = JSON.stringify({ title, entries, exportedAt: new Date().toISOString() }, null, 2);
                      ...
                      >
                        ...
                        {currentSession?.id === session.id && (
                          <div className="mt-2 flex items-center gap-1">
                            ...
                            <button
                              onClick={() => exportAsJSON(session.entries, session.title)}
                            ...
                            <button
                              onClick={() => exportAsCSV(session.entries, session.title)}
                            ...
                            <button
                              onClick={() => exportAsText(session.entries, session.title)}
              ...
              {currentSession ? (
                ...
                <div className="flex items-center justify-center h-full text-gray-500 dark:text-gray-400">
                  <div className="text-center">
                    ...
                    <p className="text-lg font-semibold mb-2">Select a Log Session</p>
                    <p className="text-sm">Choose an existing session or create a new one</p>
                  </div>
                </div>
              )}
            </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                        Supports: JSON, Docker, Syslog, Nginx, Apache, and generic log formats
                      </p>
                    </div>
                  </div>
...
const LogContent: React.FC<LogContentProps> = ({ session }) => {
  ...
  const { toggleLevelCollapse, toggleTimestampFold, setLevelFilter, setSearchFilter } =
    useLogStore();
  ...
  const filteredEntries = useMemo(() => {
    return session.entries.filter((entry: LogEntry) => {
      ...
      if (
        ...
        !entry.message.toLowerCase().includes(session.filters.search.toLowerCase())
      ) {
        return false;
      }
  ...
  const allLevels: LogLevel[] = ['fatal', 'error', 'warn', 'info', 'debug', 'trace', 'unknown'];
    ...
    <>
      ...
      <div className="border-b border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-800">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="relative group">
            ...
            <div className="absolute right-0 mt-1 hidden group-hover:block bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-lg z-10">
              <button
                onClick={() => exportAsJSON(filteredEntries, `${session.title}_filtered`)}
              ...
              <button
                onClick={() => exportAsCSV(filteredEntries, `${session.title}_filtered`)}
                className="block w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-900 dark:text-white whitespace-nowrap"
              >
                Export as CSV
              </button>
              <button
                onClick={() => exportAsText(filteredEntries, `${session.title}_filtered`)}
                className="block w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-900 dark:text-white whitespace-nowrap"
              >
                Export as TXT
              </button>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {allLevels.map((level) => {
            ...
            const count = session.entries.filter((e: LogEntry) => e.level === level).length;
              ...
              <button
                ...
                onClick={() => {
                  const newLevels = isActive
                    ? session.filters.levels.filter((l: LogLevel) => l !== level)
      ...
      <div className="flex-1 overflow-y-auto bg-white dark:bg-gray-900 font-mono text-sm">
        {allLevels.map((level) => {
          ...
          return (
            <div key={level} className="border-b border-gray-200 dark:border-gray-700">
              {/* Level header */}
              <button
                ...
                entries.map((entry: LogEntry) => (
                  ...
                  >
                    <div className="flex items-start gap-3 text-gray-800 dark:text-gray-200">
                      ...
                      <pre className="flex-1 whitespace-pre-wrap break-words">{entry.message}</pre>
                    </div>
                  </div>
                ))}
            </div>
