...
export const KeyboardDebugger: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      document.removeEventListener('toggle-keyboard-debugger', toggleListener as EventListener);
    };
  ...
  const debuggerContent = !visible ? (
    ...
    >
      ...
      <div className="mb-2">
        <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Last Key:</div>
      ...
      <div>
        <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Recent (last 10):</div>
        <div className="text-xs font-mono space-y-0.5 max-h-32 overflow-y-auto">
          {events.length === 0 ? (
            ...
            events.map((key, i) => (
              <div key={i} className="bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">
                {key}
              </div>
            ))
          )}
