...
import { Monitor, RotateCcw, X, Check, AlertCircle, Info, Move, Maximize2 } from 'lucide-react';
...
export default function WindowPositionSettings({ onClose }: WindowPositionSettingsProps) {
  ...
  const loadWindowState = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load window state');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const saveCurrentState = async () => {
    try {
      ...
      setSaveStatus('error');
      setError(err instanceof Error ? err.message : 'Failed to save window state');
  ...
  const restoreState = async () => {
    try {
      ...
      setSaveStatus('error');
      setError(err instanceof Error ? err.message : 'Failed to restore window state');
      setTimeout(() => setSaveStatus('idle'), 3000);
    }
  };
  ...
  const resetToDefault = async () => {
    try {
      ...
      setSaveStatus('error');
      setError(err instanceof Error ? err.message : 'Failed to reset window state');
      setTimeout(() => setSaveStatus('idle'), 3000);
    }
  };
    ...
    <div className="w-[550px] bg-[#1a1b26] border border-[#414868] rounded-lg shadow-2xl">
      ...
      <div className="flex items-center justify-between p-4 border-b border-[#414868]">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-[#c0caf5]">Window Position Memory</h2>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      <div className="p-4">
          ...
          <div className="mb-4">
            ...
            {saveStatus === 'success' && (
              <div className="flex items-center gap-2 text-[#9ece6a]">
                ...
                <span className="text-sm">Window state updated successfully</span>
              </div>
            )}
            {saveStatus === 'error' && (
        ...
        <div className="mb-6 p-3 bg-[#7aa2f7]/10 border border-[#7aa2f7]/30 rounded-lg">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-[#c0caf5]">
              <strong>Automatic Memory:</strong> The app automatically remembers your window
              position and size when you move or resize it. Use the controls below to manually
              manage the saved state.
            </div>
          </div>
        </div>
        ...
        {windowState && (
          <div className="mb-6">
            <h3 className="text-sm font-medium text-[#c0caf5] mb-3">Current Window State</h3>
            <div className="grid grid-cols-2 gap-4">
              {/* Position */}
              <div className="bg-[#24283b]/50 rounded-lg p-3 border border-[#414868]">
                <div className="flex items-center gap-2 mb-2">
                  <Move className="w-4 h-4 text-[#9ece6a]" />
                  <span className="text-sm font-medium text-[#c0caf5]">Position</span>
                </div>
              ...
              <div className="bg-[#24283b]/50 rounded-lg p-3 border border-[#414868]">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-[#c0caf5]">Size</span>
                </div>
                <div className="text-sm text-[#9aa5ce]">
            ...
            <div className="mt-3 bg-[#24283b]/50 rounded-lg p-3 border border-[#414868]">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-[#c0caf5]">Maximized State</span>
                <span
        ...
        <div className="mt-6 pt-4 border-t border-[#414868]">
          <div className="text-xs text-[#9aa5ce]">
            <strong>How it works:</strong> The app automatically saves your window position and size
            whenever you move or resize the window. On next startup, it will restore the last saved
            state. You can manually save, restore, or reset the window position using the buttons
            above.
          </div>
        </div>
      </div>
