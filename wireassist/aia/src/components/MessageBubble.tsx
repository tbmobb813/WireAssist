...
export default function MessageBubble({ message, isHighlighted = false }: Props) {
  ...
  const getArtifactsForMessage = useArtifactStore((s) => s.getArtifactsForMessage);
  ...
  const handleFork = () => {
    try {
      ...
      const messageIndex = messages.findIndex((m: ApiMessage) => m.id === message.id);
      ...
      if (branchId) {
        useBranchStore.getState().switchBranch(currentConversation.id, branchId);
        addToast({
