...
export default function ChatInterface(): JSX.Element {
  const { currentConversation, messages, sendMessage, isLoading } = useChatStore();
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('insert-command', handleInsertCommand as any);
  }, []);
        ...
        if (clipText) {
          ...
          if (looksLikeCode && !clipText.startsWith('```')) {
            ...
            if (
              ...
              language = 'javascript';
            } else if (clipText.includes('def ') || clipText.includes('import ')) {
              language = 'python';
            } else if (clipText.includes('fn ') || clipText.includes('let mut ')) {
              language = 'rust';
            }
    ...
    await withErrorHandling(
      async () => {
        ...
        // Code patterns
          ...
          // Detect language
          let language = '';
          if (clipText.includes('function') || clipText.includes('=>')) language = 'javascript';
        ...
        // Shell command
          ...
          clipText.startsWith('$') ||
          clipText.startsWith('#') ||
          clipText.match(/^(npm|yarn|pnpm|cargo|git|docker|kubectl|python|node)/)
        ) {
  ...
  const { vimMode } = useVimNavigation({
    ...
    onPrevConversation: () => {
      const { conversations, currentConversation, selectConversation } = useChatStore.getState();
                    ...
                    <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-lg bg-gradient-to-r from-[#9ece6a]/20 to-[#73daca]/20 border border-[#9ece6a]/30">
                      ...
                      <span
                        className={`w-2 h-2 rounded-full ${
                          gitContext.dirty ? 'bg-[#f7768e] animate-pulse' : 'bg-[#9ece6a]'
                        }`}
                        title={gitContext.dirty ? 'Uncommitted changes' : 'Clean working directory'}
                      />
                    </div>
                  )}
      ...
      >
        ...
        {!isLoading &&
          visibleMessages.map((m: Message) => (
            <MessageBubble key={m.id} message={m} isHighlighted={selectedMessageId === m.id} />
          ))}
        {!isLoading && visibleMessages.length === 0 && (
          <FadeIn delay={300}>
            <div className="text-center py-12">
              ...
              <h4 className="text-lg font-medium text-[#c0caf5] mb-2">Start the Conversation</h4>
              <p className="text-sm text-[#9aa5ce] max-w-sm mx-auto">
                Ask a question, request help with code, or start a discussion. Your AI assistant is
                ready to help!
              </p>
            </div>
          </FadeIn>
            ...
            {slashSuggestions.map((command, index) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    <div className="font-medium text-[#7aa2f7] text-sm">{command.name}</div>
                    <div className="text-xs text-[#9aa5ce] mt-1">{command.description}</div>
                    {command.parameters && (
