...
function CodeBlock({ inline, className, children, ...props }: CodeProps) {
  ...
  const handleSave = async () => {
    const filename = prompt('Enter filename:', language ? `code.${language}` : 'code.txt');
    if (!filename) return;
  ...
  const isRunnable = ['bash', 'sh', 'zsh', 'python', 'node', 'javascript'].includes(
        ...
        >
          ...
          <button
            ...
            className={`px-2 py-1 rounded-lg hover:bg-gray-700 transition-all duration-150 text-xs ${
              copied ? 'text-green-400 bg-green-900/30' : 'text-gray-400 hover:text-gray-200'
            }`}
            title="Copy code"
