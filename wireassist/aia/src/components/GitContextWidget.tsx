...
export default function GitContextWidget({ projectPath, onIncludeContext }: GitContextWidgetProps) {
      ...
      <div className="rounded-xl bg-gradient-to-br from-[#7aa2f7]/10 to-[#bb9af7]/10 backdrop-blur-sm border border-[#7aa2f7]/30 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1">
              {projectInfo && projectInfo.project_type !== 'Unknown' && (
                <span className="text-2xl">{getProjectIcon(projectInfo.project_type)}</span>
              )}
          ...
          <svg
            ...
            className={`text-[#9aa5ce] transition-transform ${isExpanded ? 'rotate-180' : ''}`}
          >
            <path d="m6 9 6 6 6-6" />
          </svg>
        </button>
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm p-2 rounded bg-[#24283b]/50">
                  <div className="flex items-center gap-2">
                    <span className="text-xl">{getProjectIcon(projectInfo.project_type)}</span>
                    <div>
                      ...
                      <div className="text-xs text-[#9aa5ce]">
                        ...
                        {projectInfo.description && (
                          <span className="ml-1">• {projectInfo.description}</span>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex items-center gap-2 text-sm">
              {gitContext.uncommitted_changes === 0 ? (
                <>
                  ...
                  <span className="text-[#c0caf5]">Working directory clean</span>
                </>
              ) : (
                <>
                  ...
                  {gitContext.recent_commits.slice(0, 3).map((commit) => (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono text-[#565f89]">{commit.hash.slice(0, 7)}</span>
                        <span className="text-[#565f89]">•</span>
                        <span className="text-[#9aa5ce]">{commit.date}</span>
                      </div>
                      <div className="text-[#c0caf5] line-clamp-2">{commit.message}</div>
                      <div className="text-[#565f89] text-xs mt-1">{commit.author}</div>
                    </div>
                  ))}
                </div>
              </div>
