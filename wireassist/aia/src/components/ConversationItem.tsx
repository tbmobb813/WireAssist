...
export default function ConversationItem({ conversation, selected, onSelect }: Props) {
  ...
  const handleExport = async (e: MouseEvent, format: 'json' | 'markdown' | 'html' | 'pdf') => {
    e.stopPropagation();
    setExporting(true);
    try {
  ...
  return (
    ...
    >
      <div className="flex-1">
        {!editing && (
          <>
            <div className="text-sm font-medium truncate">{conversation.title || 'Untitled'}</div>
            <div className="text-xs text-gray-400 truncate">{conversation.model}</div>
          </>
        )}
        ...
        {editing && (
          <form onSubmit={handleRename} className="flex gap-2">
            <input
              ...
              autoFocus
            />
            <button type="submit" className="text-xs px-2 py-1 bg-green-600 rounded">
              Save
            </button>
            <button
              type="button"
