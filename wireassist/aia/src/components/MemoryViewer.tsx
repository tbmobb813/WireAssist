...
export const MemoryViewer: React.FC = () => {
  ...
  const filteredMemories = useMemo(() => {
    ...
    if (!searchQuery.trim()) {
      return [...projectMemory.entries].sort((a, b) => b.createdAt - a.createdAt);
    }
    ...
    <FadeIn>
      <div className="fixed inset-y-0 right-0 z-50 w-96 bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700 shadow-2xl flex flex-col">
        ...
        <div className="flex gap-2 px-4 py-3 border-b border-gray-200 dark:border-gray-700">
          ...
          <AnimatedButton onClick={handleExport} variant="secondary" size="sm" className="flex-1">
            <Download className="w-3 h-3 mr-1" />
            Export
          </AnimatedButton>
          <AnimatedButton
            onClick={() => {
              if (confirm(`Clear all memories for ${projectPath}? This cannot be undone.`)) {
        ...
        {/* Add Memory Modal */}
        {showAddModal && <AddMemoryModal onClose={() => setShowAddModal(false)} />}
      </div>
    </FadeIn>
  );
};
    ...
    <div className="bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden hover:border-purple-300 dark:hover:border-purple-600 transition-colors">
      ...
      <div className="flex items-start gap-2 p-3 cursor-pointer" onClick={onToggleExpand}>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm text-gray-900 dark:text-white line-clamp-2">{memory.content}</p>
        ...
        <div className="px-3 pb-3 space-y-2 border-t border-gray-200 dark:border-gray-700 pt-3">
          {memory.context && (
            <div className="text-xs">
              <span className="font-semibold text-gray-700 dark:text-gray-300">Context:</span>
              <p className="text-gray-600 dark:text-gray-400 mt-1">{memory.context}</p>
            </div>
          )}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-md">
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Add Memory</h3>
              <button
                onClick={onClose}
            ...
            <div className="flex gap-3 mt-6">
              <AnimatedButton onClick={handleAdd} variant="primary" size="md" className="flex-1">
                Add Memory
              </AnimatedButton>
