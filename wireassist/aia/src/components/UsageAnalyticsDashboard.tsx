...
export default function UsageAnalyticsDashboard({ onClose }: UsageAnalyticsDashboardProps) {
  const [data, setData] = useState<AnalyticsData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [dateRange, setDateRange] = useState<'week' | 'month' | 'year' | 'all'>('month');
  const [selectedMetric, setSelectedMetric] = useState<'conversations' | 'messages'>(
    'conversations'
  );
  ...
  const calculateAnalytics = async (
    ...
    const filteredConversations = conversations.filter((c) => c.created_at > cutoffTime);
    ...
    <div className="p-6 bg-[#1a1b26] min-h-screen">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#9aa5ce]">Insights into your AI Assistant usage and performance</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          <div className="h-48 flex items-end space-x-1">
            {data.dailyActivity.map((day, index) => {
              const value = selectedMetric === 'conversations' ? day.conversations : day.messages;
              const maxValue = Math.max(
                ...data.dailyActivity.map((d) =>
                  selectedMetric === 'conversations' ? d.conversations : d.messages
                )
              );
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-gray-900 dark:text-white">
                  {data.messagesByRole.assistant > 0
                    ? (data.messagesByRole.user / data.messagesByRole.assistant).toFixed(2)
                    : 'N/A'}
                </span>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="text-lg font-medium text-gray-900 dark:text-white">
                {new Date(data.performanceMetrics.peakUsageDay).toLocaleDateString()}
              </div>
            </div>
            <div>
              <span className="text-sm text-gray-600 dark:text-gray-400">Most Active Hour</span>
              <div className="text-lg font-medium text-gray-900 dark:text-white">
            ...
            {Object.entries(data.modelStats)
              ...
              .map(([model, count]) => (
                <div key={model} className="flex items-center justify-between">
                  <span className="text-sm text-gray-600 dark:text-gray-400 truncate">{model}</span>
