...
export default function CostDashboard() {
  ...
  const budgetPercent = budgetMonthly > 0 ? (currentMonthSpend / budgetMonthly) * 100 : 0;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-gradient-to-br from-[#9ece6a]/10 to-[#9ece6a]/5 border border-[#9ece6a]/30 rounded-lg p-3">
          ...
          <div className="text-2xl font-bold text-[#c0caf5] font-mono">{formatCost(totalCost)}</div>
        </div>
      </div>
        ...
        <div className="bg-gradient-to-br from-[#73daca]/10 to-[#73daca]/5 border border-[#73daca]/30 rounded-lg p-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-[#73daca]">
              ...
              <span className="text-xs font-medium">Cost Savings (via Smart Routing)</span>
            </div>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            >
              ...
              <span className="text-xs font-medium">This Month: {currentMonth}</span>
            </div>
            <div
          ...
          <div className="w-full bg-[#24283b] rounded-full h-2">
            <div
              className={`h-2 rounded-full transition-all ${
                overBudget ? 'bg-[#f7768e]' : budgetPercent > 80 ? 'bg-[#e0af68]' : 'bg-[#9ece6a]'
              }`}
        ...
        <div>
          ...
          <div className="space-y-2">
            {monthlySpends.map((month) => {
              ...
              const maxSpend = Math.max(...monthlySpends.map((m) => m.cost), budgetMonthly);
              const widthPercent = maxSpend > 0 ? (month.cost / maxSpend) * 100 : 0;
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    >
                      {month.month}
                      {isCurrentMonth && (
                        <span className="ml-2 text-xs text-[#9aa5ce]">(current)</span>
                      )}
                    </span>
        ...
        <div>
          ...
          <div className="space-y-2">
            {modelUsageList.slice(0, 5).map((item, idx) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-[#c0caf5] font-medium">{item.model}</span>
                  <span className="text-xs text-[#9aa5ce] font-mono">
      ...
      {topConversations.length > 0 && (
        <div>
          <div className="flex items-center gap-2 text-[#9aa5ce] mb-3">
            <TrendingUp size={14} />
            <span className="text-xs font-semibold uppercase tracking-wide">Top Conversations</span>
          </div>
          <div className="space-y-2">
            {topConversations.map((conv) => (
              <div
                ...
                className="bg-[#24283b] border border-[#414868] rounded-lg p-3 hover:border-[#565f89] transition-colors"
              >
                <div className="text-sm text-[#c0caf5] font-medium truncate mb-2">{conv.title}</div>
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-3">
                    <span className="text-[#7aa2f7] font-mono">{formatTokens(conv.tokens)}</span>
                    <span className="text-[#565f89]">•</span>
                    <span className="text-[#9aa5ce]">{conv.messageCount} msgs</span>
                  </div>
      ...
      {totalTokens > 0 && (
        <div className="bg-[#24283b]/50 border border-[#414868]/50 rounded-lg p-3">
          ...
          <div className="text-xs text-[#9aa5ce] space-y-1">
            <div>
              Average: {formatCost(totalCost / Math.max(conversations.length, 1))} per conversation
            </div>
