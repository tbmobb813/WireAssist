...
const SECTION_KEYS = ['usage-cost', 'project-files', 'git-status', 'quick-actions', 'recent-files'];
...
export default function ContextPanel() {
  ...
  const toggleAll = () => {
    ...
    SECTION_KEYS.forEach((key) => {
      try {
        localStorage.setItem(`context-section:${key}`, newState ? 'open' : 'closed');
      } catch {
...
function Section({ icon, title, children, defaultOpen = true, storageKey }: SectionProps) {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(`context-section:${storageKey}`, isOpen ? 'open' : 'closed');
    } catch {
      // ignore storage errors
    }
  }, [isOpen, storageKey]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('context-panel-toggle-all', handleToggleAll);
  }, []);
  ...
  return (
    <div className="border-b border-gray-800">
      ...
      >
        ...
        <ChevronRight
          size={14}
          className={`transform transition-transform-smooth ${isOpen ? 'rotate-90' : ''}`}
        />
      </button>
...
function ProjectFilesSection() {
  ...
  if (events.length === 0) {
    return (
      <Section icon={<Folder size={16} />} title="Project Files" storageKey="project-files">
  ...
  return (
    <Section icon={<Folder size={16} />} title="Project Files" storageKey="project-files">
      <div className="space-y-1 text-sm">
        {fileTree.length > 0 ? (
          fileTree.map((item, idx) => <FileTreeItem key={idx} item={item} depth={0} />)
        ) : (
  ...
  if (item.type === 'file') {
    return (
      ...
      >
        ...
        <span className="text-gray-400 hover:text-gray-200 truncate">{item.name}</span>
        {item.modified && <span className="ml-auto w-1.5 h-1.5 bg-yellow-500 rounded-full"></span>}
      </div>
    );
  }
...
function GitStatusSection() {
  ...
  if (!gitContext?.is_repo) {
    return (
      <Section icon={<GitBranch size={16} />} title="Git Status" storageKey="git-status">
    ...
    <Section icon={<GitBranch size={16} />} title="Git Status" storageKey="git-status">
      <div className="space-y-2 text-sm">
        <div className="flex items-center justify-between text-gray-300">
          <span>Branch:</span>
          <span className="font-mono text-blue-400">{gitContext.branch || 'main'}</span>
        </div>
...
function QuickActionsSection() {
  ...
  return (
    <Section icon={<Zap size={16} />} title="Quick Actions" storageKey="quick-actions">
...
function RecentFilesSection() {
  const { events } = useProjectStore();
  const [files, setFiles] = useState<Array<{ name: string; time: string; path?: string }>>([]);
  ...
  return (
    <Section icon={<FileText size={16} />} title="Recent Files" storageKey="recent-files">
      <div className="space-y-2 text-sm">
        {files.map((file, idx) => (
          <button
            key={idx}
