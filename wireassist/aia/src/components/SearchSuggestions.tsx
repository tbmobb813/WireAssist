...
export const SearchSuggestions: React.FC<SearchSuggestionsProps> = ({
  ...
  const addRecentSearchSuggestions = useCallback(
  ...
  const addPopularTagSuggestions = useCallback(
    (suggestions: SearchSuggestion[]) => {
  ...
  const addMatchingConversations = useCallback(
    (suggestions: SearchSuggestion[], q: string) => {
  ...
  const addMatchingTags = useCallback(
    (suggestions: SearchSuggestion[], q: string) => {
  ...
  const addSearchCommands = useCallback((suggestions: SearchSuggestion[], q: string) => {
    const commands = [
      {
        cmd: 'from:',
    ...
    const matching = commands.filter((cmd) => cmd.cmd.startsWith(q) || q.startsWith(cmd.cmd));
  ...
  const addFilterSuggestions = useCallback((suggestions: SearchSuggestion[], q: string) => {
    const filters = [
    ...
    const matching = filters.filter((filter) => filter.text.includes(q) && filter.text !== q);
  ...
  const handleSuggestionClick = (suggestion: SearchSuggestion) => {
    ...
    if (suggestion.type !== 'conversation' && !suggestion.text.startsWith('tag:')) {
      const updated = [
        suggestion.text,
    ...
    >
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 px-3 py-2">
        <div className="text-xs text-gray-500 dark:text-gray-400">
          ...
          <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded">tag:name</code> or{' '}
          <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded">from:user</code> for advanced
          filtering
        </div>
      </div>
    </div>
