    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-11/12 max-w-3xl bg-[#1a1b26] border border-[#414868] rounded-lg shadow-2xl p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-sm font-semibold text-[#c0caf5]">Execution Result</h3>
        ...
        <div className="space-y-3">
          ...
          <div className="text-sm text-[#9aa5ce]">
            Exit code:{' '}
            <span className="text-[#c0caf5] font-medium">{String(runModal.exit_code ?? '—')}</span>
            {runModal.timed_out ? <span className="text-[#f7768e]"> (timed out)</span> : ''}
          </div>
        </div>
      </div>
    </div>
