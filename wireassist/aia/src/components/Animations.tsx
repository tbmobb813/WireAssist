...
export const AnimatedButton: React.FC<AnimatedButtonProps> = ({
  ...
  const variants = {
    primary: 'bg-blue-500/20 hover:bg-blue-500/30 text-blue-200 border-blue-400/30',
...
export const Pulse: React.FC<PulseProps> = ({
  ...
  return <div className={`${isActive ? intensities[intensity] : ''} ${className}`}>{children}</div>;
};
  ...
  return (
    <div className={`w-full ${className}`}>
      <div className="flex justify-between items-center mb-2">
        {showPercentage && <span className="text-sm text-white/60">{Math.round(progress)}%</span>}
      </div>
...
export const Skeleton: React.FC<SkeletonProps> = ({
  ...
  if (variant === 'circular') {
    return <div className={`${baseClasses} rounded-full ${className}`} style={{ width, height }} />;
  }
