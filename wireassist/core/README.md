...
Core AI engine for **WireAssist** — multi-provider AI with privacy controls, streaming, and the agent platform (MCP, approvals, memory, events).

> Package name: `@wireassist/core` (path: `wireassist/core/`). Older docs may refer to `@aia/core` or `@synqworks/core`.
...
```bash
pnpm add @wireassist/core --workspace
```

From another project (when published):

```bash
pnpm add @wireassist/core
```
...
```typescript
import { CoreStreamingProvider, SearchService, PrivacyService } from '@aia/core';
...
```typescript
import { CoreStreamingProvider, ContextAwareProvider } from '@aia/core';
...
```typescript
try {
  const response = await provider.generateResponse('conv-123', messages, (chunk) =>
    console.log(chunk)
  );
} catch (error) {
